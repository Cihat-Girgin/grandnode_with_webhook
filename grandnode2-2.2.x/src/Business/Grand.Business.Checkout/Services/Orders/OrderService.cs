...
public class OrderService : IOrderService
{
    ...
    ///     Gets orders by code
    /// </summary>
    /// <param name="idempotencyKey">The order idempotencyKey</param>
    /// <returns>Order</returns>
    public virtual async Task<Order> GetOrderByIdempotencyKey(string idempotencyKey)
    {
        if (string.IsNullOrEmpty(idempotencyKey))
            return null;

        return await _orderRepository.GetOneAsync(o=>o.IdempotancyKey == idempotencyKey);
    }
    ...
    ///     Warehouse identifier, only orders with products from a specified warehouse will be loaded; 0
    ///     to load all orders
    /// </param>
    ...
    /// <param name="orderGuid">
    ///     Search by order GUID (Global unique identifier) or part of GUID. Leave empty to load all
    ///     orders.
    /// </param>
    ...
    /// <summary>
    ///     Inserts an order AND RETURN CREATED ORDER ID
    /// </summary>
    /// <param name="order">Order</param>
    public virtual async Task<Order> InsertWebHookOrder(Order order)
    {
        ArgumentNullException.ThrowIfNull(order);
    ...
    public async Task CancelExpiredOrders(DateTime expirationDateUtc)
    {
        var orders = _orderRepository.Table
            .Where(o => o.CreatedOnUtc < expirationDateUtc && o.PaymentStatusId == PaymentStatus.Pending &&
                        ...
                        (o.ShippingStatusId == ShippingStatus.Pending
