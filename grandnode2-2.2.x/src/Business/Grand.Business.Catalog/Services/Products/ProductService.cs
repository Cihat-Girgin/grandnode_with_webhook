...
public class ProductService : IProductService
{
    ...
    public virtual async Task<Product> GetProductBySku(string sku)
    {
        ...
        Builders<Product>.Filter.In(p => p.Sku, skuList);
        var all = _productRepository.Table.Where(p => skuList.Contains(p.Sku));
        return await _productRepository.GetOneAsync(x => x.Sku == sku);
    }

    public virtual async Task<List<Product>> GetProductsBySkuList(string[] skuList)
    {
        var query =  _productRepository.Table.Where(p => skuList.Contains(p.Sku));
